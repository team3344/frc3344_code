...
public:
	...
	int AutonomousTask()
	{
		...
		double straightProfile[] = {0.65, 0.6, 0.60, 0.60, 0.35, 0.35, 0.35, 0.0};
		...
		while ((time = timer->Get()) < 8.0 && !atCross) {
			...
			int leftValue = lsState | LightSensorArray::kLeftOn;	// read the line tracking sensors
			int middleValue = lsState | LightSensorArray::kMidOn;
			int rightValue = lsState | LightSensorArray::kRightOn;
			...
			if (goLeft) {	//	FIXME: conversion to binary value ahere is a waste
